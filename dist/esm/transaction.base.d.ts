...
export declare class RedisXTransactionBase<
	L extends unknown[] = [],
	F = unknown,
	U = undefined,
> {
	#private;
	protected redisTransaction: import("@redis/client/dist/lib/client/multi-command").RedisClientMultiCommandType<
		RedisModules,
		RedisFunctions,
		RedisScripts
	>;
	constructor(
		redisTransaction: ReturnType<
			RedisClientType<RedisModules, RedisFunctions, RedisScripts>["MULTI"]
		>,
	);
	/**
	 ...
	 */
	addCommand(
		...command_arguments: (string | number)[]
	): RedisXTransaction<[...L, unknown], F, U>;
	/**
	 * Assigns the result of the last command to a key in the result object.
	 * @param key Key to assign the result to.
	 * @returns -
	 */
	as<K extends string, T = L extends [...infer _, infer T] ? T : never>(
		key: K,
	): RedisXTransaction<
		L,
		string extends K ? F : F & { [key in K]: T },
		string extends K ? (undefined extends U ? T : U | T) : U
	>;
	/**
	 ...
	 */
	execute(): Promise<
		never[] & L & (undefined extends U ? F : F & Partial<Record<string, U>>)
	>;
	/**
	 * @protected
	 * @param module -
